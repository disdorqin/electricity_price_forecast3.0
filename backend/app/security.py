...
async def require_ops(request: Request) -> None:
    """Ops endpoints require access (key/local) AND must be enabled unless local."""
    await require_access(request)
    if settings.ops_enabled:
        return
    # Disabled: only localhost may use ops endpoints.
    if is_local(client_host(request)):
        return
    raise HTTPException(
        status_code=status.HTTP_403_FORBIDDEN,
        detail="Ops endpoints are disabled. Set ops_enabled=true or access from localhost.",
    )
