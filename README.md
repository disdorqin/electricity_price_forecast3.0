...
本仓库聚合了多个电力负荷/电价时序预测模型的实现。包含三块内容:
...
> 三个模型独立运行、互相独立,共享相同的输入字段约定(参见 [`docs/metrics_calculation.md`](./docs/metrics_calculation.md))。
...
`python-dotenv` / `openpyxl` / `chinese-calendar` / `borax`。
