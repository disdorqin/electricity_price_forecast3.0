...
def test_confirm_true_passes_guard(client: TestClient):
    # With confirm=true the guard passes; it then fails only on missing DB (503),
    # proving confirm is honored rather than silently blocked.
    r = client.post("/api/ops/export-submission", json={"target_date": "2026-01-01", "confirm": True})
    assert r.status_code == 503  # DB not configured, not a confirm error
